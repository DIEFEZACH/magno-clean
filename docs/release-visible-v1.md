...
| Componente / alcance | Antes | Con remediación parcial |
| --- | --- | --- |
| Backend, `npm audit --json` | 4 altas + 3 moderadas | 3 altas, 0 moderadas |
| Backend, `npm audit --omit=dev --json` | 4 altas + 3 moderadas | 3 altas, 0 moderadas |
| Frontend, ambos comandos | 0 | 0 |

El exit code **1** de los audits backend significa vulnerabilidades detectadas, no fallo de ejecución. Son **cinco avisos independientes**, no siete: `@prisma/config`, `prisma`, `body-parser` y `express` heredan avisos de sus dependencias. Después del parche queda **un aviso independiente** (`deepmerge-ts`), reflejado en tres entradas altas (`deepmerge-ts`, `@prisma/config`, `prisma`). No se suprime ni acepta ese riesgo.
...
### Justificación de los dos overrides
...
Los overrides están acotados a **Express → qs 6.16.0** y **Prisma → mysql2 3.23.1**, no a cualquier paquete del árbol. No cambia la versión directa de Express, Prisma, Prisma Client ni adapter-pg. El lockfile sólo cambia esos dos paquetes y la sustitución transitiva necesaria de mysql2: sale `seq-queue`/`sqlstring`, entra `sql-escaper@1.5.1`. Las versiones Node requeridas siguen siendo compatibles con Node 22. No se usa `npm audit fix --force`, actualización general ni override del major deepmerge 8.
...
El residual de deepmerge se mantiene visible. Mantener configuración Prisma revisada/no recursiva y evitar alimentar configuraciones con objetos no confiables reduce la exposición observada; **no sustituye el parche ni acepta el riesgo en nombre del propietario**. Una actualización mayor necesita evaluar las API/semántica utilizadas por Prisma/c12 o esperar una versión compatible del padre. No añadir un override 7→8 sin esa autorización.
...
Validación local: instalación reproducible `npm ci` en ambos componentes, Prisma generate/validate **7.10.0**, backend build y **269 tests** (262 anteriores + 7 regresiones de dependencias), frontend **181 tests**, **63 tests** de scripts/configuración/sitemap, lint y build:ci. El Worker compilado pasó **10 casos mock** de rutas/auth/assets bajo bloqueo de red. Los nuevos tests verifican ambas resoluciones qs (Express y body-parser), ambos avisos qs, consultas/JSON Express, rechazo de clear-password antes de leer una contraseña, API promise/escaping de mysql2 y descompresión con paquetes minúsculos válidos e inválidos. No abren conexiones DB ni prueban con datos reales. Las pruebas de protocolo usan helpers internos de mysql2: revisarlas explícitamente al retirar o actualizar el override.

### Decisión y secuencia productiva conservada

El SHA **b37e82979d140af2d9597993217356289e6c0ea2** permanece congelado. Este PR no lo sustituye automáticamente: cualquier merge futuro exige un SHA nuevo y validación posterior. **El gate de dependencias sigue pendiente por deepmerge; CI verde no concede GO productivo.** Safari real, indexación de staging y P2 visuales siguen separados; no cambiar cookies, Remote Automation o configuración remota en este lote.
