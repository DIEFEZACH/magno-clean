  ...
  return (
    <section className="bg-[#F5F5F5] px-5 py-20 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        {!loading && !error && featured.length > 0 && <div className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-3">{featured.map((product) => <ProductCard key={product.id} product={{ ...product, oldPrice: product.oldPrice ?? undefined, badge: product.badge ?? undefined }} />)}</div>}
